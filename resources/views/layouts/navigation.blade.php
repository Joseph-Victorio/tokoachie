<div x-data="{ open: false }" class="relative md:h-auto flex bg-background">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        <a href="{{ route('review.index') }}" @click="open = false" class="block text-lg py-2 hover:underline">Review
            Pembeli</a>
        <a href="{{ route('pembeli.index') }}" @click="open = false" class="block text-lg py-2 hover:underline">Review
            Pembeli</a>
        <a href="https://dashboard.sandbox.midtrans.com/" @click="open = false" class="block text-lg py-2 hover:underline">Data Penjualan</a>
    ...
    <aside class="hidden sm:flex sm:flex-col w-64 bg-primary border-r shadow-md p-6 space-y-6">
        ...
        <nav class="flex flex-col space-y-2 font-anek">
            ...
            <x-nav-link href="https://dashboard.sandbox.midtrans.com/" >
                {{ __('Data Penjualan') }}
            </x-nav-link>
    ...
    <div class="flex-1 flex flex-col">
        ...
        <!-- Topbar (mobile only) -->
        <header class="sm:hidden flex items-center justify-between bg-background border-b shadow p-4">
            <a href="{{ route('dashboard') }}">
                <p class="font-sansita text-primary">Toko Achie</p>
            </a>
